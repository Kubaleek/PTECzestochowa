...
export default function MapsContent() {
  ...
  return (
    <section>
      <motion.h2 ref={headerRef} variants={headerVariants} initial="hidden" animate={headerInView ? "visible" : "hidden"} transition={{ duration: 0.8 }} className="flex items-center mb-9">
      ...
      <div className="grid grid-cols-12 gap-6">
        ...
        <div className="col-span-12 lg:col-span-8 xl:col-span-9 items-center justify-center flex flex-col gap-6">
          ...
          <motion.div ref={buttonsRef} className="flex flex-wrap justify-center gap-3 lg:w-3/4" variants={buttonVariants} initial="hidden" animate={buttonsInView ? "visible" : "hidden"} transition={{ duration: 0.6, staggerChildren: 0.2 }}>
            {Object.keys(cityContacts).map((city) => (
              <motion.button key={city} type="button" className={`border px-2 py-1 text-[12px] rounded-lg lg:hover:scale-110 transition-all ease duration-300 ${selectedCity === city     ? "border-[#f79c1d] bg-white"     : "border-[#17822e] hover:border-[#f79c1d] hover:bg-white" }`} onClick={() => handleCityClick(city)} variants={buttonVariants}>
