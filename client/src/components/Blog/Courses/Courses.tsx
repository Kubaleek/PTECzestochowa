...
const CoursesLogin = () => {
  const methods = useForm<LoginFormInputs | RegisterFormInputs>();
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const { mutate: doRegister } = useRegisterMutation({
    onSuccess: () => console.log("Rejestracja przebiegła pomyślnie"),
    onError: (error) =>
  ...
  return (
    <FormProvider {...methods}>
      <div className="w-full bg-white shadow-lg flex flex-col rounded border border-black/25">
        <div className="w-full bg-green-700 p-4 flex flex-col gap-3 text-white">
          <Header />
          <hr />
          <p className="text-xs text-pretty leading-relaxed text-justify">
          Witaj! Aby uzyskać dostęp do listy szkoleń w systemie PTE Częstochowa, musisz najpierw zarejestrować swoje konto. Po pomyślnej rejestracji będziesz mógł się zalogować, korzystać z panelu szkoleń, przeglądać aktualne szkolenia oraz wiele więcej.
          </p>
        </div>
        <div className="p-4 bg-[#f7f3ef]">
          <Tabs
            aria-label="Options"
            color="primary"
            variant="underlined"
              ...
              >
                ...
                <div>
                  <Button
                    ...
                    color="success"
                    variant="flat"
                    className="w-auto rounded !text-white bg-green-700"
                    disabled={isLoading}
                  >
...
export default function Courses() {
  ...
  return <>{session && session.user ? <Panel /> : <CoursesLogin />}</>;
}
