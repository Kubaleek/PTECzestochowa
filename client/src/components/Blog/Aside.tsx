...
const Aside = () => {
  ...
  // Get the current id from the query parameters
  ...
  // Filter navigation items by category based on the current pathname
  const filterNavItemsByCategory = (items: NavItem[], pathname: string) => {
    const categorySlug = pathname.split("/")[1]; // Extract the category from the path (e.g., 'aktualnosci')
    return items.filter(
            ...
            {Object.entries(groupedNavItems).map(([category, items]) => {
              ...
              return (
                <div key={category} id={category} className="flex flex-col gap-2">
                  {filteredItems.map((e) => {
                    const itemUrl = `/${slugify(e.category.toLowerCase())}?id=${e.id}`;
                    const isActive = pathname === `/${slugify(e.category.toLowerCase())}` && currentIdNumber === e.id;
